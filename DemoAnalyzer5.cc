...
u = u+1;
   ...
   cout << "  => position of vertex no "<<u<< " in z = " << vertices ->position().z() << " && in rho = " << vertices -> position().Rho() << endl;
cout <<"----------------------------------------------------------------------------------" <<endl;   
//cout <<"-------------------------------------------------------------------" <<endl;  
}
u =0;
//       if(!primary_vertices->isFake()) {
}
...
if (track2 -> quality(reco::Track::qualityByName("highPurity"))                  // 1- high quality
     ...
     && abs(track2 ->dz())/ track2 ->dzError() < 3.0                              // 5- dz/ σ(dz) < 3
     && abs(track2 ->dxy())/ track2 ->dxyError() < 3.0    ) 
j = j+1;
}
...
histoSl ->Fill(j);
cout <<"*** the numer of tracks after selection in this event = " <<j <<endl;
j=0;
